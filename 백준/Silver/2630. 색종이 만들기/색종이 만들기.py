...
# 크기가 1이 되거나 반대 색이 발견되지 않은 경우, 맞는 전역변수에 더하고 리턴.
def cutting(cp, ci, cj, end):
    ...
    for i in range(ci, ci + end):
        if (not base_color) in cp[i][cj : cj + end]:
            cutting(cp, ci, cj, end // 2)   # 2 사분면                  2 1
            cutting(cp, ci + end // 2, cj, end // 2)    # 3 사분면      3 4
            cutting(cp, ci, cj + end // 2, end // 2)    # 1 사분면
            cutting(cp, ci + end // 2, cj + end // 2, end // 2) # 4 사분면
            return
